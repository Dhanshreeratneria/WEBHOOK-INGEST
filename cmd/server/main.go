...
func main() {
	...
	// srv.Shutdown only waits for in-flight HTTP requests. Recording
	// processing runs detached in its own goroutine after the response is
	// sent, so it needs its own wait here or it gets killed mid-flight.
	if err := svc.Shutdown(shutdownCtx); err != nil {
		log.Error("in-flight recording processing did not finish before shutdown", "err", err)
	}
}
