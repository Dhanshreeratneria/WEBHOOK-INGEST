...
// InsertEvent stores the raw delivery and reports whether a new row was
// actually inserted. false with a nil error means event_id was already
// present — this delivery is a duplicate/redelivery and the caller should
// skip further processing.
//
// This is the source of truth for idempotency. The unique constraint on
// events.event_id (migrations/002_dedupe_events.sql) plus ON CONFLICT DO
// NOTHING make the check-and-insert a single atomic operation in Postgres,
// so it is race-free even when two redeliveries of the same event_id arrive
// concurrently — unlike a separate "SELECT exists" followed by "INSERT".
